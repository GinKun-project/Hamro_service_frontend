...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    if (!token) {
      setToastError('Invalid or missing reset token');
    }
  }, [token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setToastError('Invalid reset token');
      return;
    }
  ...
  if (!token) {
    return (
      <div className="w-full min-h-screen flex items-center justify-center p-6 relative z-[1]">
        <AuthCard logo="Hamro Service" title="Invalid Link" subtitle="">
          <div className="text-center flex flex-col gap-4">
            <p>The password reset link is invalid or has expired.</p>
            <Link href="/forgot-password" className="text-[var(--text-orange)] no-underline font-medium transition-[var(--transition)] hover:text-[var(--text-orange-dark)] hover:underline">
  ...
  if (success) {
    return (
      <div className="w-full min-h-screen flex items-center justify-center p-6 relative z-[1]">
        <AuthCard logo="Hamro Service" title="Password Reset Successful" subtitle="">
          <div className="text-center flex flex-col gap-4">
  ...
  return (
    <div className="w-full min-h-screen flex items-center justify-center p-6 relative z-[1]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center max-w-[1200px] w-full">
        <AuthCard logo="Hamro Service" title="Reset Password" subtitle="">
